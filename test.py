...
def getListings(eventId):
    ...
    for listingStart in range(100, numListings, 100):
        ...
        tempResponse = requests.get('https://www.stubhub.com/bfx/api/search/inventory/v2/listings', headers=headers,
                                    params=params).json()
        jsonListings.extend(tempResponse['listing'])
...
sequentialResults = getListings(103812348)
print(f"Sequential time took {time.time() - sequentialStartTime}")

pool = mp.Pool(mp.cpu_count())
parallelStartTime = time.time()
parallelResults = getListingsParallel(103812348, pool)

print(f"Parallel time took {time.time() - parallelStartTime}")
